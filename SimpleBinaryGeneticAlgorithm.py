...
class SimpleBinaryGenetic:
    ...
    def __init__(self, n_chromosome: int, m_gene: int, genes_lengths: np.ndarray, genes_intervals: np.ndarray,
                 target_function, fitness_function: FitnessFunctions, p_crossover: float = 0.9,
                 p_mutation: float = 0.005, is_maximization=True, use_linear_ranking=False, use_sigma_limited=False,
        ...
        self.fitness_function: FitnessFunctions = fitness_function
    ...
    def begin_learning(self, iteration: int = 100):
        ...
        axs[0].plot(range(draw_begin, iteration), max_fitnesses[draw_begin:],  label=f'best so far {i}')

        axs[0].set_xlabel('iteration')
        axs[0].set_ylabel('fitness value')

        all_chromosomes = self.get_target_niche()
    ...
    def evaluate_target_function(self, decoded_chromosomes: np.ndarray):
        ...
        if self.__use_linear_ranking:
            ...
            np_sum = np.sum(self.fitness)
            self._selection_probability = self.fitness /np_sum
